// src/api.js — FamilyVault API service
// All server communication goes through here. Import and call these in your components.

const BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:3000/api";

// ─── Token Storage ────────────────────────────────────────────────────────────
// Kept in memory only (not localStorage) to reduce XSS risk.
// On page refresh the user will need to log in again — acceptable for a secure vault.
...
export async function register(email, password) {
  // Role is no longer accepted — server always assigns "owner" on registration
...
function scheduleTokenRefresh() {
  ...
  _refreshTimer = setTimeout(async () => {
    try {
      ...
      clearToken(); // Force re-login if refresh fails
    }
  }, 13 * 60 * 1000);
}
